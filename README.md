...
already written. In domain mode you also need `CHAT_DOMAIN`,
`CHAT_BASE_URL`, the chat origin in `CORS_ALLOWED_ORIGINS`, and
`cp Caddyfile.chat.template Caddyfile`.

Use the installer's `restart` for this, not a raw `docker compose down`
(with or without `--remove-orphans`) — Compose filters teardown to the
profiles currently active, so once `.env` no longer selects chat, plain
`down` can't see that container and silently leaves it running.
`--remove-orphans` doesn't help either: it only removes services dropped
from the compose file entirely, not one that's merely profile-gated off.
`restart` names the chat profile explicitly so it really is removed; the
manual equivalent is `docker compose stop chat && docker compose rm -f chat`.

Hand-editing `COMPOSE_PROFILES=chat` yourself works too, in either
direction: the next `cortex update` detects it and corrects `cortex.json`
to match, rather than reverting your edit back to whatever it last knew.
